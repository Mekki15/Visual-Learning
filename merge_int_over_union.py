...
if __name__== '__main__':
   ...
   for mod,label in zip(models_list,labels_list):
       ...
       for item in selective_features:
            #print 'selective_feature is', item[0], 'hopa', item[1][0], 'hopa',item[1][1] 
	 name=item[0].replace('.png','').split('_')[-1]
         if int(name)%5==0:
            res=mod.predict(item[1][0])
           # print res
            for feat in truth_features: 
	      #print 'truth feature is',feat[0],'toto',feat[1][0],'toto',feat[1][1],'toto',feat[1][2]

              ## if they refer to same image
	      if feat[0]==item[0]:
                 #print 'feat is',feat[0],'item is' ,item[0]
                 truth_boxes=feat[1][2]
                 selec_boxes=item [1][1]
                # print type(truth_boxes[0][0])
                # print type(selec_boxes[0][0])
...
#                    print item[0],feat[0][0]
                 #print feat[0][0]
                 #pirnt item[0]  
                 if 1 in res and not label in feat[1][0]:
#                    print 'feature label',feat[1][0]      
                   # print label,'_false positive'
                    for i,j in enumerate(res):
#                      print 'item feature is', item[1][0][i]
                      if int(j)==1: #false_positives
                        Y_truth.append(0)
                        Y_score.append(1)
                        x_test.append(item[1][0][i])
                      else:         #true_negatives
                        Y_truth.append(0)
                        Y_score.append(0)
                        x_test.append(item[1][0][i])
                 #print 'feature is' , feat[1][0]
                 #print 'label is',label
                 if  1 in res and  label in feat[1][0]:
                    prediction=[]
                    for i,j in enumerate(res):
                      if int(j)==1: 
                        predict=selec_boxes[i]
                        for x,y in enumerate(feat[1][0]):
                          if y==label:  
                             box=truth_boxes[x]
#                             print 'predict',predict,'box',box
                            # IOU(predict,box)
...
#       ax.plot() 
       plt.savefig('only5_merge2_label'+str(label)+'.png')   # save the figure to file                      
